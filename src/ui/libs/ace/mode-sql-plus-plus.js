...
(function() {
  ...
  var keywords = (
      "adapter|advise|all|alter|analytics|analyze|and|any|apply|args|as|asc|at|autogenerated|between|btree|bucket|by|cascade|case|cast|catalog|closed|collection|compact|compaction|connect|connected|container|copy|correlate|create|cross|cube|current|database|dataset|dataverse|declare|definition|delete|desc|describe|disable|disconnect|distinct|div|drop|element|else|enable|end|enforced|every|except|exclude|execute|exists|explain|external|feed|filter|first|flatten|following|for|foreign|from|full|fulltext|function|grant|group|grouping|groups|having|hints|if|ignore|in|include|index|ingestion|inner|insert|internal|intersect|into|is|join|key|keyword|known|last|left|let|letting|like|limit|link|load|missing|mod|ngram|no|nodegroup|not|null|nulls|offset|on|open|or|order|others|outer|output|over|partition|path|policy|pre-sorted|preceding|primary|range|raw|references|refresh|replace|respect|return|returning|returns|revoke|right|role|rollup|row|rows|rtree|run|satisfies|scope|secondary|select|set|sets|snapshot|some|source|start|statistics|stop|synonym|temporary|then|ties|timestamp|to|transform|truncate|type|unbounded|union all|union|unknown|unnest|update|upsert|use|user|using|value|valued|view|when|where|with|write"
  );
